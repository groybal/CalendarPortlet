 ...
 * allows users to view, add, delete and edit all available calendars.
 *
 * @author Jen Bourey
 */
@Controller
...
public class EditCalendarSubscriptionsController {
  ...
  public ModelAndView viewSubscriptions(RenderRequest request, RenderResponse response) {
    ...
    String subscribeId =
        (String) session.getAttribute(SessionSetupInitializationService.USERNAME_KEY);
    ...
    List<UserDefinedCalendarConfiguration> mycalendars =
        calendarStore.getUserDefinedCalendarConfigurations(subscribeId, false);
    model.put("mycalendars", mycalendars);

    // add the predefined calendars to the model
    List<PredefinedCalendarConfiguration> calendars =
        calendarSetDao.getAvailablePredefinedCalendarConfigurations(request);
    ...
    List<PredefinedCalendarDefinition> definitions =
        calendarStore.getHiddenPredefinedCalendarDefinitions(subscribeId, userRoles);
  ...
  public void removeSubscription(
      ActionRequest request, ActionResponse response, @RequestParam("configurationId") Long id) {
  ...
  public void addSharedCalendar(
      ActionRequest request, ActionResponse response, @RequestParam("definitionId") Long id) {
    PortletSession session = request.getPortletSession();
    String subscribeId =
        (String) session.getAttribute(SessionSetupInitializationService.USERNAME_KEY);
    PredefinedCalendarDefinition definition =
        (PredefinedCalendarDefinition) calendarStore.getCalendarDefinition(id);
  ...
  public void updatePreferences(
      ...
      @ModelAttribute(FORM_NAME) CalendarPreferencesCommand form)
      throws Exception {
