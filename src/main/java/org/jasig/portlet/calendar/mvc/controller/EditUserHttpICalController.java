...
public class EditUserHttpICalController {
  ...
  public void updateHttpCalendar(
    ...
    if (form.getId() > -1) {
      ...
      config =
          (UserDefinedCalendarConfiguration) calendarStore.getCalendarConfiguration(form.getId());
  ...
  protected UserHttpIcalCalendarForm getHttpCalendarForm(PortletRequest request) {
    ...
    if (id != null && !id.equals("")) {
      ...
      if (configurationId > -1) {
        CalendarConfiguration listing =
            (CalendarConfiguration) calendarStore.getCalendarConfiguration(configurationId);
